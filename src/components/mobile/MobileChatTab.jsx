...
import { Loader2, Send, Plus, Trash2, ChevronLeft, Bot } from 'lucide-react';
import MessageBubble from '@/components/agents/MessageBubble';

const AGENT_NAME = 'planning_assistent';

export default function MobileChatTab({ startNew = false }) {
  ...
  const hasStartedNew = useRef(false);
  ...
  // Auto-start new conversation when startNew prop is true
  useEffect(() => {
    if (startNew && !hasStartedNew.current && !loadingConversations) {
      hasStartedNew.current = true;
      createNewConversation();
    }
  }, [startNew, loadingConversations]);
...
GEBRUIKER INFO:
- Bedrijf: ${currentCompany?.name || 'Onbekend'} (companyId: ${currentCompany?.id || ''})
- Naam: ${employeeInfo} (employeeId: ${employeeId})
- Email: ${user?.email || ''}

DATUM INFO:
- Vandaag: ${dayNames[dow]} ${currentDate}
...
- Begroet bij voornaam. Toon NOOIT ID's. Wees bondig.`;
  ...
  const loadConversations = async () => {
    try {
      const allConvos = await base44.agents.listConversations({ agent_name: AGENT_NAME });
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 8rem)' }}>
      <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ borderBottom: '1px solid var(--color-border)' }}>
        <button onClick={() => { setShowList(true); setCurrentConversation(null); }} className="p-1">
